...
export default function YearViewCore({
  ...
  const handleQuickAddSubmit = useCallback(
    async (title: string) => {
      ...
      setQuickAddError(null);
      try {
        const created = await eventApi.createEvent(targetCalendar, { title, date });
        setEvents((prev) => [...prev, created]);
        setQuickAddCell(null);
        focusYearGrid();
        // Reconcile with the source so colors/ordering match the stored copy.
        loadData(year);
      } catch (err) {
    ...
    [eventApi, focusYearGrid, loadData, quickAddCell, setEvents, targetCalendar, year],
  );
  ...
  const handleEditSubmit = useCallback(
    async (title: string) => {
      ...
      setEditError(null);
      try {
        await eventApi.updateEvent(target.calendarId, target.id, { title });
        setEvents((prev) =>
    ...
    [editTarget, eventApi, focusYearGrid, loadData, setEvents, year],
  );
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    setDeleteError(null);
    try {
      await eventApi.deleteEvent(target.calendarId, target.id);
  ...
  }, [deleteTarget, eventApi, focusYearGrid, loadData, setEvents, year]);
